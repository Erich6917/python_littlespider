...
class mainAll(object):
    ...
    def scarpy_start(self, driver):
        ...
        for province, citys in dic_city.items():
            ...
            for city in citys:
                ...
                # file_name = province_path + '/' + decode(city) + '.txt'
                # try:
                #     self.file_company = open(file_name, 'a')
                #     # 主函数，遍历每个城市的行业分类、注册资本、注册时间、企业状态
                #     self.tel_city_trade(province, city, driver)
                # except Exception, e:
                #     self.file_error.write('遍历失败:' + city + str(e))
                # finally:
                #     self.file_company.close()
    ...
    def sleeps():
        ...
        # time.sleep(random.randint(1, 5)) #OK
    ...
    def find_trade_total(self, driver):
        """
                    获取行业下的总条数
        """
        list_title = driver.find_elements_by_xpath("//*[@id='web-content']/div/div/div/div/div/div/span")
        if list_title:
            try:
                msg = ''
